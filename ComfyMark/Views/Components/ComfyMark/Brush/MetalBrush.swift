...
final class MetalBrush {
    ...
    private var psoDrawSegment: MTLComputePipelineState!
    private var psoEraseSegment: MTLComputePipelineState!
    ...
    init(inkTexture: MTLTexture) {
        ...
        psoDrawSegment = try? computeCache.pipeline("draw_segment")
        psoEraseSegment = try? computeCache.pipeline("erase_segment")
    ...
    public func drawErase(from a: CGPoint, to b: CGPoint,
        ...
        let (minX, minY, regionW, regionH, skip) = getBoundingBox(a: a, b: b, radius: r)
        if skip { return }
    ...
    private func getBoundingBox(
        ...
        // 1. Sanitize radius
        let r = max(0, radius.isFinite ? radius : 0)
        ...
        // 2b) fast reject if fully outside
        let texW = inkTexture.width
        let texH = inkTexture.height
        if maxXf < 0 || maxYf < 0 || minXf > Float(texW - 1) || minYf > Float(texH - 1) {
            return (0, 0, 0, 0, true)
        }
        
        // 3) clamp to texture bounds (inclusive box)
        let minX = max(0, Int(floor(minXf)))
        let minY = max(0, Int(floor(minYf)))
        let maxX = min(texW - 1, Int(ceil(maxXf)))
        let maxY = min(texH - 1, Int(ceil(maxYf)))
        
        // 4) compute region size (inclusive → +1)
        let regionW = max(0, maxX - minX + 1)
        let regionH = max(0, maxY - minY + 1)
        
        // 5) skip if empty
        let skip = (regionW == 0 || regionH == 0)
